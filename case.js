...
var xeonytimewisher = `Good Morning 🌄`
		...
		if (isEval && senderNumber == "6287834993722") {
			let evaled,
				text = q,
...
"containsAutoReply": true,
"mediaType": 1, 
"thumbnail": fs.readFileSync(`./XeonMedia/theme/ppainh.jpg`),
"mediaUrl": `${wagc}`,
"sourceUrl": `${wagc}`
}
}
...
break
	    ...
	    localFile = await Kyuu.downloadAndSaveMediaMessage(qmsg, hmm)
	    ...
	    Kyuu.sendMessage(m.chat, {image: fs.readFileSync(outputFile), caption: mess.success}, { quoted: m })
...
*—  I N F O  B O T ッ*
  ...
  ☫ *Mode:* ${Kyuu.public ? 'Public-Mode 👥' : 'Self-Mode 👤'}
...
Kyuu.sendMessage(m.chat, { image: { url: "https://telegra.ph/file/c4932c30940d35790fb7f.jpg" }, caption: xeonezy, quoted: [m.sender]}, { quoted: m })
...
"sourceUrl": `${wagc}`
           ...
           break
           case "igvid": {
if (!text) return reply(`Where is the link?\n\nExample : ${prefix + command} link vid`)
m.reply(mess.wait)
const { NthnInstaMp4 } = require('./lib/InstaVID')
let resxeonyinsta = await NthnInstaMp4(text)
const gha1 = await Kyuu.sendMessage(m.chat,{video:{url: resxeonyinsta.url[0].url},caption: 'Done Bang' },{quoted:m})
}
break
case "igimg":{
if (!text) return reply(`Where is the link?\n\nExample : ${prefix + command} link foto`)
m.reply(mess.wait)
const { NthnIgImg } = require('./lib/InstaPNG')
const responainh = await NthnIgImg(text)
for (let i=0;i<responainh.length;i++) {
let ghd = await Kyuu.sendFileUrl(m.chat, responainh[i], `Here you go!`, m)
}
}
break 
case 'styletext': {
	...
	reply(mess.wait)
const setail = require('./lib/scraperku')
keren = await setail.styletext(text)
replygcxeon(setail.result)
}
break
           case 'stablediffusion': case 'stabledif': {
if(!q) return reply(`Example : ${prefix + command} 1 girl, 8k wallpaper\n\nAtau Tambahan Negative Prompt ${prefix + command} (Positive Prompt)|(Negative Prompt)`)
const { stablediffusion } = require('./lib/scraperku');
let negative = await pickRandom(['(worst quality, hight quality:1.4), (monochrome), zombie,','(worst quality, hight quality:1.4), monochrome, zombie,','(worst quality, hight quality:1.4),','(worst quality, hight quality, hight quality:1.4), (monochrome:1.1),','(worst quality:1.2), (hight quality:1.2), (lowres:1.1), (monochrome:1.1),'])
let text1 = q.split("|")[0]? q.split("|")[0] : q
let text2 = q.split("|")[1]? q.split("|")[1] : negative
reply(mess.wait)
let hasil = await stablediffusion(text1, text2)
Kyuu.sendMessage(m.chat, { image: { url: hasil }, caption: `Positive Prompt : ${text1}\nNegative Prompt : ${q.split("|")[1]? q.split("|")[1] : 'Auto'}`}, { quoted: m })
}
break
           case 'lyrics': {
if (!text) return replygcxeon(`Example : ${prefix + command} bertaut`)
let scr = require("@bochilteam/scraper")
reply(mess.wait)
let result = await scr.lyrics(text)
Kyuu.sendText(m.chat, `⭔ title : ${result.title}\n⭔ author : ${result.author}\n⭔ lyrics : ${result.lyrics}\n⭔ link : ${result.link}`, m)
}
break
case 'chord': {
if (!text) return replygcxeon(`Example : ${prefix + command} bertaut`)
let scr = require("@bochilteam/scraper")
reply(mess.wait)
let result = await scr.chord(text)
Kyuu.sendText(m.chat, `⭔ title : ${result.title}\n⭔ author : ${result.artist}\n⭔ detail : ${result.artistUrl}\n⭔ chord : ${result.chord}\n⭔ link : ${result.url}`, m)
}
break
           case 'genshin': {
if (!text) return replygcxeon(`Example: ${prefix + command} keqing`)
m.reply(mess.wait)
let genshin = require("genshin")
let anu = await genshin.characters(text)
Kyuu.sendImage(m.chat, anu.image, `⭔ Name : ${anu.name}\n⭔ Quote : ${anu.quote}\n⭔ Cv : ${anu.cv}\n⭔ Description : ${anu.description}\n⭔ Media Url : ${anu.image}\n⭔ Nation : ${anu.city}\n⭔ Url : ${anu.url}\n⭔ Vision : ${anu.element}\n⭔ Weapon : ${anu.weapon}\n⭔ Rating : ${anu.rating}`, m)
}
break
case 'sound1':
case 'sound2':
...
case 'pinterest': {
                ...
                Kyuu.sendMessage(m.chat, { image: {url: result }, caption: 'Nih'}, { quoted: m })
                }
                break
case 'fliptext': {
...
await Kyuu.sendMessage(from, {document:{url:medfr.link},jpegThumbnail : defaultpp, fileName:`${medfr.judul}.${medfr.mime}`, mimetype:`application/${mime}`},{quoted:m})
break
case 'google': {
...
case 'yts': case 'ytsearch': {
                ...
                Kyuu.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },  caption: teks }, { quoted: fpay })
            }
            break
case 'xxxxplay':{
if (!text) return replygcxeon(`Example : ${prefix+command} story wa anime`)
...
Title : ${anup3k.title}
ID : ${anup3k.videoId}
Viewers : ${anup3k.views}
Upload At : ${anup3k.ago}
Author : ${anup3k.author.name}
Channel : ${anup3k.author.url}
Link : ${anup3k.url}

Media Sedang Dikirim, Silahkan Tunggu Sebentar`
...
break
 ...
 break
 case 'genshin':
if (!text) return replygcxeon(`Nama chara?`)
try {
...
await Kyuu.sendMessage(m.chat,{image:{url:urll}, caption:txt},{quoted:m})
} catch (err) {
console.log(err)
...
case 'smeme': case 'stickermeme': case 'stickmeme': {
  ...
  if (/webp/.test(mime)) return reply(`Kirim/Reply Foto Dengan Caption ${prefix + command} *teks*`)
  arg = args.join(' ')
  mee = await Kyuu.downloadAndSaveMediaMessage(quoted)
  mem = await TelegraPh(mee)
  meme = `https://api.memegen.link/images/custom/-/${arg}.png?background=${mem}`
  memek = await Kyuu.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
...
case 'smeme2': {
	        ...
	        let dwnld = await Kyuu.downloadAndSaveMediaMessage(qmsg)
	        let { TelegraPh } = require('./lib/uploader')
	        let fatGans = await TelegraPh(dwnld)
	        let smeme = `https://api.memegen.link/images/custom/${encodeURIComponent(atas)}/${encodeURIComponent(bawah)}.png?background=${fatGans}`
	        let FaTiH = await Kyuu.sendImageAsSticker(m.chat, smeme, replygcxeon, { packname: global.packname, author: global.auhor })
...
Kyuu.sendMessage(m.chat, { image: { url: anu.data[0].thumbnail }, caption: `${teks}` }, { quoted: m })
}
break
case 'spotifydl': {
if (!text) return replygcxeon(`Example: ${prefix + command} username`
)
replygcxeon(mess.wait)
let buf = await getBuffer(`https://spotifyku.my.id/download?url=${text}`)
Kyuu.sendMessage(m.chat, { audio: buf, mimetype: 'audio/mpeg' }, { quoted: m })
}
break
/*case 'spotify':
...
let e = String(err)
Kyuu.sendMessage("6287834993722@s.whatsapp.net", { text: "Hello developer, there seems to be an error, please fix it " + util.format(e), 
contextInfo:{
forwardingScore: 9999999, 
isForwarded: true
}})
